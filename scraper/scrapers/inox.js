// INOX Cinemas scraper — Playwright + Stealth
// Scrapes cinema/venue/showtime data from INOX Mumbai.
// Prices on INOX are only visible after selecting a showtime (seat layout),
 ...
 * INOX pricing data appears when the user clicks a showtime and the
...
/**
 * Step 1: Get movie links from the INOX Mumbai page.
 */
async function getMovieLinks(page) {
  console.log('[INOX] Navigating to movies listing...');
  await page.goto('https://www.inoxmovies.com/mumbai', {
  ...
  const movies = await page.evaluate(() => {
    const cards = document.querySelectorAll('a[href*="/movie/"], a[href*="/movies/"]');
    const results = [];
    const seen = new Set();
  ...
  console.log(`[INOX] Found ${movies.length} movie links.`);
...
async function getShowtimesPage(page, movie) {
  try {
    const url = movie.href.startsWith('http')
      ? movie.href
      : `https://www.inoxmovies.com${movie.href}`;

    console.log(`[INOX] Visiting movie: ${movie.title}`);
    ...
    if (bookBtn) {
      console.log(`[INOX] Clicking "Book" / "Showtimes" for ${movie.title}...`);
      await bookBtn.click();
      await page.waitForTimeout(3000);
      return true;
    } else {
      console.log(`[INOX] No "Book" button found for ${movie.title}, staying on page.`);
      return true; // INOX may show showtimes inline
    }
  } catch (err) {
    console.log(`[INOX] Error navigating to ${movie.title}: ${err.message}`);
    return false;
  }
}
...
async function scrapeShowtimesPage(page, movieTitle) {
  ...
  try {
    const venueData = await page.evaluate(() => {
      ...
      if (entries.length === 0) {
        ...
        for (const el of allSpans) {
          ...
          if (
            (text.startsWith('INOX') || text.startsWith('PVR') ||
    ...
    console.log(`[INOX] Error scraping showtimes page: ${err.message}`);
  }
...
async function clickShowtimeForPricing(page, intercepted) {
  try {
    ...
    if (!showtimeButton) {
      console.log('[INOX] No clickable showtime buttons with time pattern found.');
      return;
    }

    const showtimeText = (await showtimeButton.textContent()).trim();
    console.log(`[INOX] Clicking showtime: ${showtimeText}`);

    await showtimeButton.click();
    await page.waitForTimeout(4000);

    if (intercepted.length > 0) {
      console.log(`[INOX] Captured ${intercepted.length} price entries from seat-layout API.`);
    } else {
      console.log('[INOX] No pricing data captured from XHR after clicking showtime.');
    }

    await page.goBack();
    await page.waitForTimeout(2000);
  } catch (err) {
    console.log(`[INOX] Error clicking showtime: ${err.message}`);
  }
}

/**
 * Main scraper function for INOX Cinemas Mumbai.
 *
 * @returns {Promise<Array>} Array of normalized price objects
 */
async function scrapeINOX() {
  let browser = null;

  try {
    console.log('[INOX] Launching browser...');
    ...
    if (movies.length === 0) {
      console.log('[INOX] No movie links found.');
      return [];
    }
    ...
    for (const movie of movies) {
      ...
      console.log(`[INOX] Found ${venueData.length} venues for ${movie.title}`);
    ...
    if (intercepted.length > 0) {
      console.log(`[INOX] Total XHR price entries: ${intercepted.length}`);
    ...
    console.log(`[INOX] Total raw results after dedup: ${deduped.length}`);
    ...
    for (const raw of deduped) {
      try {
        ...
        const entry = normalizePrice(raw, raw.cinema || 'INOX');
        validateSchema(entry);
        normalized.push(entry);
      } catch (err) {
        console.log(`[INOX] Skipping invalid entry: ${err.message}`);
      }
    }

    console.log(`[INOX] Final normalized results: ${normalized.length}`);
    return normalized;
  } catch (err) {
    console.error('[INOX] Scraper error:', err.message);
    return [];
  } finally {
    if (browser) {
      await browser.close();
      console.log('[INOX] Browser closed.');
    }
  }
}

module.exports = { scrapeINOX };

// Run directly for testing
if (require.main === module) {
  scrapeINOX().then((results) => {
    console.log('\n[INOX] Final Results:');
