// Cinepolis Cinemas scraper — Playwright + Stealth
// Scrapes cinema/venue/showtime data from Cinepolis India.
// Prices on Cinepolis are only visible after selecting a showtime (seat layout),
 ...
 * Cinepolis pricing data appears when the user clicks a showtime and the
...
/**
 * Step 1: Get movie links from the Cinepolis page.
 */
async function getMovieLinks(page) {
  console.log('[Cinepolis] Navigating to movies listing...');
  await page.goto('https://www.cinepoliscinemas.in/', {
  ...
  // Try to select Mumbai city if a city selector is present
  try {
    const citySelector = await page.$('a:has-text("Mumbai"), button:has-text("Mumbai"), [data-city="mumbai"], span:has-text("Mumbai")');
    if (citySelector) {
      console.log('[Cinepolis] Selecting Mumbai city...');
      await citySelector.click();
      await page.waitForTimeout(2000 + Math.random() * 3000);
    }
  } catch (_) {
    // City may already be selected or selector not present
  }

  await page.waitForTimeout(3000);

  const movies = await page.evaluate(() => {
    const cards = document.querySelectorAll('a[href*="/movie/"], a[href*="/movies/"], a[href*="/film/"]');
    const results = [];
    const seen = new Set();
    ...
    for (const card of cards) {
      ...
      if (!title) {
        ...
        const moviesIdx = segments.findIndex((s) => s === 'movies' || s === 'movie' || s === 'film');
        if (moviesIdx >= 0) {
  ...
  console.log(`[Cinepolis] Found ${movies.length} movie links.`);
...
async function getShowtimesPage(page, movie) {
  try {
    const url = movie.href.startsWith('http')
      ? movie.href
      : `https://www.cinepoliscinemas.in${movie.href}`;

    console.log(`[Cinepolis] Visiting movie: ${movie.title}`);
    ...
    if (bookBtn) {
      console.log(`[Cinepolis] Clicking "Book" / "Showtimes" for ${movie.title}...`);
      await bookBtn.click();
      await page.waitForTimeout(3000);
      return true;
    } else {
      console.log(`[Cinepolis] No "Book" button found for ${movie.title}, staying on page.`);
      return true; // Cinepolis may show showtimes inline
    }
  } catch (err) {
    console.log(`[Cinepolis] Error navigating to ${movie.title}: ${err.message}`);
    return false;
  }
}
...
async function scrapeShowtimesPage(page, movieTitle) {
  ...
  try {
    const venueData = await page.evaluate(() => {
      ...
      if (entries.length === 0) {
        ...
        for (const el of allSpans) {
          ...
          if (
            (text.startsWith('Cinepolis') || text.startsWith('Cinépolis') ||
              text.startsWith('PVR') || text.startsWith('INOX') ||
              text.startsWith('MovieMax') || text.startsWith('Carnival')) &&
    ...
    console.log(`[Cinepolis] Error scraping showtimes page: ${err.message}`);
  }
...
async function clickShowtimeForPricing(page, intercepted) {
  try {
    ...
    if (!showtimeButton) {
      console.log('[Cinepolis] No clickable showtime buttons with time pattern found.');
      return;
    }

    const showtimeText = (await showtimeButton.textContent()).trim();
    console.log(`[Cinepolis] Clicking showtime: ${showtimeText}`);

    await showtimeButton.click();
    await page.waitForTimeout(4000);

    if (intercepted.length > 0) {
      console.log(`[Cinepolis] Captured ${intercepted.length} price entries from seat-layout API.`);
    } else {
      console.log('[Cinepolis] No pricing data captured from XHR after clicking showtime.');
    }

    await page.goBack();
    await page.waitForTimeout(2000);
  } catch (err) {
    console.log(`[Cinepolis] Error clicking showtime: ${err.message}`);
  }
}

/**
 * Main scraper function for Cinepolis Cinemas India.
 *
...
async function scrapeCinepolis() {
  ...
  try {
    console.log('[Cinepolis] Launching browser...');
    ...
    if (movies.length === 0) {
      console.log('[Cinepolis] No movie links found.');
      return [];
    }
    ...
    for (const movie of movies) {
      ...
      console.log(`[Cinepolis] Found ${venueData.length} venues for ${movie.title}`);
    ...
    if (intercepted.length > 0) {
      console.log(`[Cinepolis] Total XHR price entries: ${intercepted.length}`);
    ...
    console.log(`[Cinepolis] Total raw results after dedup: ${deduped.length}`);
    ...
    for (const raw of deduped) {
      try {
        ...
        const entry = normalizePrice(raw, raw.cinema || 'Cinepolis');
        validateSchema(entry);
        normalized.push(entry);
      } catch (err) {
        console.log(`[Cinepolis] Skipping invalid entry: ${err.message}`);
      }
    }

    console.log(`[Cinepolis] Final normalized results: ${normalized.length}`);
    return normalized;
  } catch (err) {
    console.error('[Cinepolis] Scraper error:', err.message);
    return [];
  } finally {
    if (browser) {
      await browser.close();
      console.log('[Cinepolis] Browser closed.');
    }
  }
}

module.exports = { scrapeCinepolis };

// Run directly for testing
if (require.main === module) {
  scrapeCinepolis().then((results) => {
    console.log('\n[Cinepolis] Final Results:');
