// PVR Cinemas scraper — Playwright + Stealth
// Scrapes cinema/venue/showtime data from PVR Mumbai.
// Prices on PVR are only visible after selecting a showtime (seat layout),
 ...
 * PVR pricing data appears when the user clicks a showtime and the
...
/**
 * Step 1: Get movie links from the PVR Mumbai page.
 */
async function getMovieLinks(page) {
  console.log('[PVR] Navigating to movies listing...');
  await page.goto('https://www.pvrcinemas.com/movies/mumbai', {
  ...
  const movies = await page.evaluate(() => {
    const cards = document.querySelectorAll('a[href*="/movies/"], a[href*="/movie/"]');
    const results = [];
    const seen = new Set();
    ...
    for (const card of cards) {
      ...
      if (!title) {
        ...
        const moviesIdx = segments.findIndex((s) => s === 'movies' || s === 'movie');
        if (moviesIdx >= 0) {
  ...
  console.log(`[PVR] Found ${movies.length} movie links.`);
...
async function getShowtimesPage(page, movie) {
  try {
    const url = movie.href.startsWith('http')
      ? movie.href
      : `https://www.pvrcinemas.com${movie.href}`;

    console.log(`[PVR] Visiting movie: ${movie.title}`);
    await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 30000 });
    await page.waitForTimeout(2000 + Math.random() * 3000);

    // Click "Book tickets" button to go to showtimes page
    const bookBtn = await page.$('button:has-text("Book"), a:has-text("Book"), button:has-text("Showtimes"), a:has-text("Showtimes")');
    if (bookBtn) {
      console.log(`[PVR] Clicking "Book" / "Showtimes" for ${movie.title}...`);
      await bookBtn.click();
      await page.waitForTimeout(3000);
      return true;
    } else {
      console.log(`[PVR] No "Book" button found for ${movie.title}, staying on page.`);
      return true; // PVR may show showtimes inline
    }
  } catch (err) {
    console.log(`[PVR] Error navigating to ${movie.title}: ${err.message}`);
    return false;
  }
}
...
async function scrapeShowtimesPage(page, movieTitle) {
  ...
  try {
    const venueData = await page.evaluate(() => {
      ...
      // Strategy 1: Look for cinema/venue elements with common PVR patterns
      const cinemaAnchors = document.querySelectorAll('a[alt], [data-cinema-name], [class*="cinema"], [class*="venue"]');
        ...
        for (const el of allSpans) {
          ...
          if (
            (text.startsWith('PVR') || text.startsWith('INOX') ||
              ...
              text.startsWith('P[') || text.includes('PVR')) &&
    ...
    console.log(`[PVR] Error scraping showtimes page: ${err.message}`);
  }
...
async function clickShowtimeForPricing(page, intercepted) {
  try {
    ...
    if (!showtimeButton) {
      console.log('[PVR] No clickable showtime buttons with time pattern found.');
      return;
    }

    const showtimeText = (await showtimeButton.textContent()).trim();
    console.log(`[PVR] Clicking showtime: ${showtimeText}`);

    await showtimeButton.click();
    await page.waitForTimeout(4000);

    if (intercepted.length > 0) {
      console.log(`[PVR] Captured ${intercepted.length} price entries from seat-layout API.`);
    } else {
      console.log('[PVR] No pricing data captured from XHR after clicking showtime.');
    }

    await page.goBack();
    await page.waitForTimeout(2000);
  } catch (err) {
    console.log(`[PVR] Error clicking showtime: ${err.message}`);
  }
}

/**
 * Main scraper function for PVR Cinemas Mumbai.
 *
 * @returns {Promise<Array>} Array of normalized price objects
 */
async function scrapePVR() {
  let browser = null;

  try {
    console.log('[PVR] Launching browser...');
    ...
    if (movies.length === 0) {
      console.log('[PVR] No movie links found.');
      return [];
    }
    ...
    for (const movie of movies) {
      ...
      console.log(`[PVR] Found ${venueData.length} venues for ${movie.title}`);
    ...
    if (intercepted.length > 0) {
      console.log(`[PVR] Total XHR price entries: ${intercepted.length}`);
    ...
    console.log(`[PVR] Total raw results after dedup: ${deduped.length}`);
    ...
    for (const raw of deduped) {
      try {
        ...
        const entry = normalizePrice(raw, raw.cinema || 'PVR');
        validateSchema(entry);
        normalized.push(entry);
      } catch (err) {
        console.log(`[PVR] Skipping invalid entry: ${err.message}`);
      }
    }

    console.log(`[PVR] Final normalized results: ${normalized.length}`);
    return normalized;
  } catch (err) {
    console.error('[PVR] Scraper error:', err.message);
    return [];
  } finally {
    if (browser) {
      await browser.close();
      console.log('[PVR] Browser closed.');
    }
  }
}

module.exports = { scrapePVR };

// Run directly for testing
if (require.main === module) {
  scrapePVR().then((results) => {
    console.log('\n[PVR] Final Results:');
